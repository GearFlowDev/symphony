...
The status TUI draws only when stdout is a terminal. Redirect it, pipe it, or run it in a
container, and Symphony writes one log line per state change instead — see
[docs/logging.md](docs/logging.md). `SYMPHONY_STATUS_BOARD=off` forces that on a terminal too;
`=on` forces the board anywhere. The web dashboard is the same in both cases.
